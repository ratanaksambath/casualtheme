...
slider.currentSlide&&!scrolling&&!(dx==null)){var target=dx>0?slider.getTarget("next"):slider.getTarget("prev");if(slider.canAdvance(target)&&Number(new Date)-startT<550&&Math.abs(dx)>20||Math.abs(dx)>cwidth/2)slider.flexAnimate(target,slider.vars.pauseOnAction);else slider.flexAnimate(slider.currentSlide,slider.vars.pauseOnAction)}this.removeEventListener("touchmove",onTouchMove,false);this.removeEventListener("touchend",onTouchEnd,false);startX=null;startY=null;dx=null;offset=null}}$(window).resize(function(){if(slider.vars.animation.toLowerCase()==
"slide")if(!slider.animating)if(slider.vertical){slider.height(slider.slides.filter(":first").height());slider.args[slider.prop]=-1*(slider.currentSlide+slider.cloneOffset)*slider.slides.filter(":first").height()+"px";if(slider.transitions){slider.setTransition(0);slider.args[slider.prop]=slider.vertical?"translate3d(0,"+slider.args[slider.prop]+",0)":"translate3d("+slider.args[slider.prop]+",0,0)"}slider.container.css(slider.args)}else{slider.newSlides.width(slider.width());slider.args[slider.prop]=
